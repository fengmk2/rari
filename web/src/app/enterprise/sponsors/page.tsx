...
function TierCard({ tier }: { tier: Tier }) {
  return (
    ...
    >
      ...
      <div
        className={`relative z-10 h-full bg-linear-to-br from-[#161b22] to-[#0d1117] border ${tier.highlight ? "border-[#fd7e14]" : "border-[#30363d]"} rounded-xl p-6 flex flex-col transition-all duration-300 group-hover:border-transparent`}
      >
          ...
          <div className="flex-1">
            <div className="space-y-3">
              {tier.benefits.map((benefit) => (
                <div
                  key={benefit.text}
                  className={`flex items-start gap-2 ${benefit.isStatement ? "mt-4 pt-4 border-t border-[#30363d]" : ""}`}
                >
                  {!benefit.isStatement && (
                    <Check className="w-5 h-5 text-[#fd7e14] shrink-0 mt-0.5" />
                  )}
                  <span
                    className={`text-gray-300 text-sm ${benefit.isStatement ? "italic text-center w-full" : ""}`}
                  >
                    {benefit.text}
                  </span>
                </div>
                  ...
                  <div className="relative z-10 h-full bg-linear-to-br from-[#161b22] to-[#0d1117] border border-[#30363d] rounded-xl p-8 transition-all duration-300 group-hover:border-transparent">
                    ...
                    <div className="relative z-10 flex flex-col items-center justify-center text-center gap-4 min-h-[120px]">
                      <div className="transform group-hover:scale-105 transition-transform duration-300 flex items-center justify-center">
                        <Icon
                          className={`${label.includes("Sanity") ? "h-8 w-auto" : "h-10 w-auto"}`}
                        />
                      </div>
