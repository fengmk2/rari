...
export default function Breadcrumbs({ pathname }: BreadcrumbsProps) {
  ...
  return (
    <nav
      ...
      className="not-prose mb-6 pt-1 pl-1 overflow-x-auto scrollbar-none"
    >
      <ol className="flex items-center space-x-2 text-sm whitespace-nowrap">
        {breadcrumbs.map((crumb, index) => {
          ...
          const uniqueKey = crumb.href
            ? `breadcrumb-${crumb.href}`
            : `breadcrumb-${crumb.label}-${breadcrumbs.length}-${isLast}`;

          return (
            <li key={uniqueKey} className="flex items-center">
              {index > 0 && <ChevronRight className="w-4 h-4 mx-2 text-gray-600" />}
