...
function registerBoundary(id, fallback, parentId) {
  ...
  if (globalThis["~suspense"].discoveredBoundaries.some((b) => b.id === id)) return;
...
globalThis["~render"].componentAsync = async function () {
  try {
    let Component =
      (globalThis["~rsc"]?.modules && globalThis["~rsc"].modules["{component_id}"]?.default) ||
    ...
    try {
        ...
        React.createElement = function (type, props, ...children) {
          const isSuspenseComponent = (type) => {
            if (typeof React !== "undefined" && React.Suspense && type === React.Suspense)
              return true;
            if (typeof type === "function" && type.name === "Suspense") return true;
          ...
          if (isSuspenseComponent(type)) {
            const boundaryId =
              ...
              props?.boundaryId ||
              props?.key ||
              `boundary_${Date.now()}_${Math.random().toString(36).substring(2, 11)}`;
            if (!globalThis["~suspense"]) globalThis["~suspense"] = {};
            ...
            return globalThis["~react"].originalCreateElement(
              "suspense",
              { ...props, key: boundaryId, boundaryId, "~boundaryId": boundaryId },
              ...children,
            );
          }
      ...
      if (isAsyncFunction) {
        ...
        const safeBoundaries = (globalThis["~suspense"].discoveredBoundaries || []).map(
          (boundary) => ({
            id: boundary.id,
      ...
      if (element && typeof element.then === "function") {
        ...
        if (!parentBoundaryId) registerBoundary(boundaryId, fallbackContent, null);
      ...
      const processSuspenseInStructure = (el, parentBoundaryId = null) => {
        ...
        if (childArray.length > 0) {
          const processedChildren = childArray.map((child) =>
            processSuspenseInStructure(child, parentBoundaryId),
          );
          return {
            ...el,
            children: Array.isArray(actualChildren) ? processedChildren : processedChildren[0],
      ...
      if (suspenseError && suspenseError.$$typeof === REACT_SUSPENSE_PENDING) {
        const componentName =
          suspenseError.componentName ||
          suspenseError.name ||
          suspenseError.message ||
          "{component_id}";
        const asyncDetected = suspenseError.asyncComponentDetected === true;
        const hasPromise =
          suspenseError.promise && typeof suspenseError.promise.then === "function";
        ...
        element = globalThis["~react"].originalCreateElement
          ? globalThis["~react"].originalCreateElement(
              "div",
              null,
    ...
    const finalResult = {
      ...
      pending_promises: globalThis["~suspense"].pendingPromises || [],
      has_suspense:
        (safeBoundaries && safeBoundaries.length > 0) ||
        (globalThis["~suspense"].pendingPromises &&
          globalThis["~suspense"].pendingPromises.length > 0),
    ...
    if (!globalThis["~render"].initialComplete) globalThis["~render"].initialComplete = true;
