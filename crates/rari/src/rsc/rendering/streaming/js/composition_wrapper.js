...
(async function () {
  try {
    ...
    if (!compositionResult) throw new Error("Composition script returned null/undefined");

    if (!compositionResult.rsc_data)
      throw new Error(
        `Composition script result missing rsc_data property. Keys: ${Object.keys(compositionResult).join(", ")}`,
      );
    ...
    if (error) {
      if (error.message) {
        ...
        try {
          ...
          if (str && str !== "[object Object]") errorMessage = str;
        } catch {}
