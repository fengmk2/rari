/* oxlint-disable no-use-before-define, no-var, vars-on-top */
...
function createJsxDelegate(runtimeJsx, globalJsx) {
  return (type, props, key) => {
    ...
    if (typeof runtimeJsx === "function") return runtimeJsx(type, props, key);
    if (typeof globalJsx === "function") return globalJsx(type, props, key);
...
if (!globalThis.readFileSync && globalThis["~node"]?.modules && globalThis["~node"].modules.get) {
  ...
  if (nodePath && nodePath.join) globalThis.join = nodePath.join;
  const nodeProcess = globalThis["~node"].modules.get("node:process");
  if (nodeProcess && nodeProcess.cwd) globalThis.cwd = nodeProcess.cwd;
}
