...
(async () => {
  try {
    if (!globalThis.getServerFunction) {
      throw new TypeError(
        "Server function registry not initialized (~serverFunctions from init_rsc_modules.js has not run)",
      );
    }

    const fn = globalThis.getServerFunction("{function_name}");

    if (!fn) throw new TypeError("Server function '{function_name}' not found or not registered");
    ...
    const processedArgs = rawArgs.map((arg) => {
      if (arg && typeof arg === "object" && !Array.isArray(arg) && !(arg instanceof FormData)) {
        const formDataLike = {
          data: arg,
          get(key) {
            if (hasOwn.call(this.data, key)) return this.data[key];
          ...
          delete(key) {
            if (hasOwn.call(this.data, key)) delete this.data[key];
          },
          entries() {
