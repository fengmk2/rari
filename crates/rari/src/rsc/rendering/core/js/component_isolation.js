...
(async function initializeComponentIsolation() {
  ...
  try {
    if (!globalThis["~components"]) globalThis["~components"] = {};
      ...
      for (const key of globalKeys) {
        ...
        if (value && typeof value.then === "function") {
          ...
          value
            ...
            .catch((error) => {
              console.error(
                `Failed to resolve global promise for key "${key}" in component context:`,
                error,
              );
            });
        }
    ...
    globalThis["~components"].isolateData = function (componentId) {
      ...
      if (!globalThis["~rsc"].componentData) globalThis["~rsc"].componentData = new Map();
