...
if (!globalThis["~rari"].lazy) {
  ...
  globalThis["~rari"].lazy.resolve = async function (promiseId) {
    ...
    try {
      ...
      if (!promiseOrDeferred) throw new Error(`Promise not found: ${promiseId}`);
      ...
      const inflightPromise = (async () => {
        ...
        if (typeof globalThis.renderToRsc === "function") {
          try {
            ...
            const rscData = await globalThis.renderToRsc(
              result,
              clientComponents,
              currentBoundaryId,
            );
            ...
            throw new Error(
              `Failed to render to RSC: ${renderError.message || String(renderError)}`,
            );
          }
        } else {
          const response = {
