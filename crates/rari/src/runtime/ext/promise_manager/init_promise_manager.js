(function initializePromiseManager() {
  ...
  if (!globalThis["~promises"].cache) globalThis["~promises"].cache = new WeakMap();
  if (!globalThis["~promises"].componentCache) globalThis["~promises"].componentCache = new Map();
  if (!globalThis["~promises"].resolved) globalThis["~promises"].resolved = new Map();
  ...
  if (!globalThis["~promises"].pending) globalThis["~promises"].pending = [];
  if (!globalThis["~promises"].failed) globalThis["~promises"].failed = new Map();
  ...
  globalThis["~promises"].wrapForIdentity = function (promise) {
    if (promise["~rari_identity"]) return promise;
  ...
  globalThis["~promises"].storeWithComponent = function (promise, result, contextId) {
    ...
    const cId = contextId || globalThis["~render"]?.currentComponent || "unknown";
  ...
  globalThis["~promises"].registerResolved = function (promise, result, componentId) {
    if (!promise || typeof promise.then !== "function") return false;

    return globalThis["~promises"].storeWithComponent(promise, result, componentId);
  };
  ...
  globalThis["~promises"].registerFunction = function (functionName, args, result) {
    ...
    if (args) {
      const signature = globalThis["~promises"].createSignatureKey(functionName, args);
      if (signature) globalThis["~promises"].functionSignatures.set(signature, result);
    }

    return true;
  };

  globalThis["~promises"].registerModuleFunction = function (moduleName, functionName, result) {
    if (!moduleName || !functionName) return false;
  ...
  globalThis["~promises"].getResolved = function (promise, componentId) {
    if (componentId && globalThis["~promises"].componentSpecific.has(componentId)) {
      const componentCache = globalThis["~promises"].componentSpecific.get(componentId);
      if (componentCache.has(promise)) return componentCache.get(promise);
    }
  ...
  globalThis["~promises"].getFunctionResult = function (functionName, args) {
    if (args) {
      const signature = globalThis["~promises"].createSignatureKey(functionName, args);
      if (signature && globalThis["~promises"].functionSignatures.has(signature)) {
  ...
  globalThis["~promises"].handleRejection = function (promise, error, componentId) {
    const cId = componentId || globalThis["~render"]?.currentComponent || "unknown";
  ...
  globalThis["~promises"].clearFailed = function (componentId) {
    if (componentId) {
      for (const [promise, data] of globalThis["~promises"].failed.entries()) {
        if (data.componentId === componentId) globalThis["~promises"].failed.delete(promise);
      }
    } else {
