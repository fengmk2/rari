...
if (typeof globalThis !== "undefined") {
  ...
  if (typeof globalThis["~rsc"].keyCounter === "undefined") globalThis["~rsc"].keyCounter = 0;
}
...
async function traverseToRsc(element, clientComponents = {}, depth = 0) {
  if (depth > 100) {
    console.error("RSC traversal depth limit exceeded, returning null to prevent stack overflow");
    return null;
  }
  ...
  if (element && typeof element === "object" && element.$$typeof === REACT_ELEMENT_TYPE) {
    return await traverseReactElement(element, clientComponents, depth + 1);
  }

  if (element && typeof element === "object" && !element.$$typeof) {
    if (element["~preSerializedSuspense"] && element.rscArray) return element.rscArray;
...
async function traverseReactElement(element, clientComponents, depth = 0) {
  ...
  if (isSuspenseComponent(type)) {
    ...
    try {
      ...
      const processedChildren = Array.isArray(props?.children) ? props.children : [props?.children];
      ...
      const hasPendingPromises = suspense.pendingPromises.some((p) => p.boundaryId === boundaryId);
      ...
      if (hasPendingPromises) {
        ...
        if (isStreaming) {
          ...
          traversedChildren =
            lazyMarkers.length === 1 ? lazyMarkers[0] : lazyMarkers.length > 0 ? lazyMarkers : null;
        } else {
          const boundaryPromises = suspense.pendingPromises.filter(
            (p) => p.boundaryId === boundaryId,
          );
          ...
          for (const pending of boundaryPromises) {
            if (pending.componentType && pending.componentProps !== undefined) {
              try {
                ...
                console.error(
                  `[rari] Error rendering async component ${pending.componentPath}:`,
                  error,
                );
          ...
          traversedChildren =
            resolvedComponents.length === 1 ? resolvedComponents[0] : resolvedComponents;
        }
      } else {
  ...
  if (typeof type === "string") {
    return await createRSCHTMLElement(type, props, uniqueKey, clientComponents, depth);
  }
...
async function createRSCHTMLElement(tagName, props, key, clientComponents, depth = 0) {
  const { children, ...otherProps } = props || {};

  const rscProps = {
    ...otherProps,
    children: children ? await traverseToRsc(children, clientComponents, depth + 1) : undefined,
  };

  if (rscProps.children === undefined || rscProps.children === null) delete rscProps.children;
...
function isClientComponent(componentType, clientComponents) {
  ...
  if (clientComponents && typeof componentType === "function") {
    ...
    if (componentName && clientComponents[componentName]) return true;
  }
  ...
  if (componentType && componentType["~isClientComponent"]) return true;

  if (typeof componentType === "string") {
    if (componentType.includes("tsx#") || componentType.includes("use client")) {
      return true;
    }
  }
...
function isServerComponent(componentType) {
  if (componentType && componentType["~isServerComponent"]) return true;
...
function getClientComponentId(componentType, clientComponents) {
  ...
  if (typeof componentType === "string") {
    ...
    if (componentNames[componentName]) return componentNames[componentName];
  ...
  const componentName = componentType.name || componentType.displayName || "UnknownClient";

  if (componentName.startsWith("$L") || componentName.startsWith("client")) return componentName;
...
function isSuspenseComponent(type) {
  ...
  if (type && type.$$typeof === REACT_SUSPENSE_TYPE) return true;
