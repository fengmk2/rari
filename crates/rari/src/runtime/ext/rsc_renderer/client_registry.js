if (typeof globalThis["~clientComponents"] === "undefined") globalThis["~clientComponents"] = {};
...
function registerClientComponent(componentId, componentPath, component) {
  ...
  if (pathName) globalThis["~clientComponentNames"][pathName] = componentId;
}
...
function isClientComponent(componentType, registry) {
  ...
  if (typeof componentType === "function") {
    ...
    if (componentName && globalThis["~clientComponentNames"][componentName]) return true;

    if (componentType["~isClientComponent"]) return true;
  }

  if (componentType && componentType.$$typeof === REACT_CLIENT_REFERENCE) return true;

  if (typeof componentType === "string" && clientRegistry[componentType]) return true;
...
function registerClientComponentFromModule(componentPath, moduleExports) {
  if (!componentPath || !moduleExports) return;
...
function markAsClientComponent(component, componentId) {
  ...
  if (componentId) component["~clientComponentId"] = componentId;
}
