...
/* eslint-disable unused-imports/no-unused-vars, no-use-before-define */
// oxlint-disable no-use-before-define
import { core, internals, primordials } from "ext:core/mod.js";
import { op_set_format_exception_callback } from "ext:core/ops";

const { getDefaultInspectOptions, getStderrNoColor, inspectArgs, quoteString } = core.loadExtScript(
...
core.registerErrorBuilder("DOMExceptionOperationError", (msg) => {
...
core.registerErrorBuilder("DOMExceptionNotSupportedError", (msg) => {
  return new DOMException(msg, "NotSupported");
});

core.registerErrorBuilder("DOMExceptionNetworkError", (msg) => {
  return new DOMException(msg, "NetworkError");
});

core.registerErrorBuilder("DOMExceptionAbortError", (msg) => {
...
core.registerErrorBuilder("DOMExceptionDataError", (msg) => {
...
function formatException(error) {
  if (core.isNativeError(error) || ObjectPrototypeIsPrototypeOf(ErrorPrototype, error)) {
    ...
    const e = inspectArgs([quoteString(error, getDefaultInspectOptions())], {
    ...
    */
    const filename = error.filename.length ? error.filename : undefined;
    const lineno = error.filename.length ? error.lineno : undefined;
