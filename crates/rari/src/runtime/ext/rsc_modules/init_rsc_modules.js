(function initializeRscModules() {
  ...
  globalThis.getServerFunction = function (name) {
    ...
    if (globalThis["~serverFunctions"].exported) {
      ...
      for (const key of exportedKeys) {
        if (
          key.endsWith(`:${name}`) &&
          typeof globalThis["~serverFunctions"].exported[key] === "function"
        ) {
          if (foundKey !== null) {
            throw new Error(
    ...
    if (globalThis["~serverFunctions"].all) {
      ...
      for (const key of allKeys) {
        if (
          key.endsWith(`:${name}`) &&
          typeof globalThis["~serverFunctions"].all[key] === "function"
        ) {
          if (foundKey !== null) {
            throw new Error(
              `Ambiguous server function name '${name}'. Multiple modules export this function: '${foundKey}' and '${key}'. Use the full namespaced key (moduleId:functionName) instead.`,
  ...
  globalThis.createServerFunctionPromise = function (functionName, args = []) {
    ...
    if (globalThis.PromiseManager && globalThis.PromiseManager.getFunction) {
      const cachedValue = globalThis.PromiseManager.getFunction(functionName, args);
    ...
    try {
      ...
      if (!serverFunction) {
        ...
        promise.toString = () => `ServerFunctionPromise(${functionName}(${JSON.stringify(args)}))`;
        return promise;
      }

      const result = serverFunction(...args);

      if (result && typeof result.then === "function") promise = result;
    ...
    promise.then(
      (value) => {
        if (globalThis.PromiseManager && globalThis.PromiseManager.registerFunction) {
