...
export const EXPORTED_FUNCTION_REGEX =
  /export\s+(?:default\s+)?(?:async\s+)?(?:function|class)[\s{(]/;
export const EXPORTED_DEFAULT_ARROW_REGEX =
  /export\s+default\s+(?:(?:async\s+)?\(|(?:async\s+)?[a-zA-Z_$][\w$]*\s*=>)/;
export const EXPORTED_CONST_FUNCTION_REGEX =
  /export\s+(?:const|let|var)\s+[a-zA-Z_$][\w$]*(?:\s*:[^;]*[^;=\s])?\s*=\s*(?:async\s+)?(?:function[\s*(]|\([^)]*\)\s*=>|[a-zA-Z_$][\w$]*\s*=>)/;
