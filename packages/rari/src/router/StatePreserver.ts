...
export class StatePreserver {
  ...
  constructor(config: StatePreserverConfig = {}) {
    ...
    this.scrollableSelector =
      config.scrollableSelector || '[data-scrollable], .scrollable, [style*="overflow"]';
  }
  ...
  private storeState(route: string, state: PreservedState): void {
    ...
    while (this.routeAccessOrder.length > this.maxHistorySize) {
      ...
      if (oldestRoute) this.stateHistory.delete(oldestRoute);
    }
  }
  ...
  public restoreState(route: string): boolean {
    ...
    if (state.focusedElement) this.restoreFocus(state.focusedElement);
            ...
            formData.forEach((value, key) => {
              try {
                ...
                if (elements instanceof RadioNodeList) {
                  ...
                  else elements.value = value as string;
                }
              } catch {
                allSucceeded = false;
              }
