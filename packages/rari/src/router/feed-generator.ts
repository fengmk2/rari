...
function generateItemXml(item: FeedEntry): string {
  ...
  if (item.pubDate) lines.push(`      <pubDate>${formatRfc822Date(item.pubDate)}</pubDate>`);
...
export function generateFeedXml(feed: Feed): string {
  ...
  const hasDcCreator = feed.items.some(
    (item) =>
  ...
  if (feed.language) lines.push(`    <language>${escapeXml(feed.language)}</language>`);

  if (feed.copyright) lines.push(`    <copyright>${escapeXml(feed.copyright)}</copyright>`);
