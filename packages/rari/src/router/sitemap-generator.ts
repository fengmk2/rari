...
function generateDetailedImageXml(image: SitemapImage): string {
  ...
  if (image.license) lines.push(`      <image:license>${escapeXml(image.license)}</image:license>`);
...
function addSitemapEntryFields(lines: string[], entry: Sitemap[number]): void {
  ...
  if (entry.lastModified) lines.push(`    <lastmod>${formatDate(entry.lastModified)}</lastmod>`);
...
function addAlternateLanguages(lines: string[], entry: Sitemap[number]): void {
  ...
  for (const [lang, url] of Object.entries(entry.alternates.languages))
    lines.push(
      `    <xhtml:link rel="alternate" hreflang="${escapeXml(lang)}" href="${escapeXml(String(url))}" />`,
    );
}
...
function extractChunkCode(result: any): string {
  ...
  const entryChunk =
    result.output.find((item: any) => item.type === "chunk" && item.isEntry) ||
