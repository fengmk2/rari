...
export class HttpRuntimeClient implements RuntimeClient {
  ...
  constructor(
    ...
    const { host = "127.0.0.1", port = 3000, timeout = 30000, ssl = false } = options;
  ...
  private async request<T = any>(
    ...
    if (body && (method === "POST" || method === "PUT")) requestOptions.body = JSON.stringify(body);
    ...
    try {
      ...
      else return (await response.text()) as T;
    } catch (error) {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      if (health.status !== "healthy") throw new Error(`Server is not healthy: ${health.status}`);
  ...
  async registerComponent(componentId: string, componentCode: string): Promise<void> {
    ...
    try {
      ...
      if (!response.success) {
        throw new Error(`Failed to register component: ${response.error || "Unknown error"}`);
      }

      if (!this.components.includes(componentId)) this.components.push(componentId);
    } catch (error) {
      throw new Error(`Failed to register component ${componentId}: ${error}`);
    }
  }

  async renderToStreamCallbacks(componentId: string, props?: string): Promise<any> {
    this.assertInitialized();
  ...
  private async queueClientComponentRegistration(
    ...
    try {
      ...
      console.error(`[rari] Runtime: Failed to register client component ${componentId}:`, error);
    }
  }
