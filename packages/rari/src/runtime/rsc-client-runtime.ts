...
import {
  createFromFetch as rariCreateFromFetch,
  createFromReadableStream as rariCreateFromReadableStream,
} from "./vendor/react-flight-client";
...
(globalThis as unknown as GlobalWithRari)["~rari"].isDevelopment =
...
if (typeof window !== "undefined") {
  if (!(window as unknown as WindowWithRari)["~rari"])
    (window as unknown as WindowWithRari)["~rari"] = (globalThis as unknown as GlobalWithRari)[
      "~rari"
    ];

  if (!(window as unknown as WindowWithRari)["~rari"].streaming)
    (window as unknown as WindowWithRari)["~rari"].streaming = {
      bufferedRows: [],
      bufferedEvents: [],
    };
...
if (typeof window !== "undefined") {
  (globalThis as unknown as GlobalWithRari)["~rari"].processBoundaryUpdate = function (
    boundaryId: string,
    rscRow: string,
    rowId: string,
  ): void {
    ...
    try {
      ...
      function containsClientComponents(element: any): boolean {
        ...
        if (Array.isArray(element)) {
          if (element.length >= 4 && element[0] === "$") {
            ...
            if (props && props.children) return containsClientComponents(props.children);
          }
      ...
      if (containsClientComponents(content)) {
        ...
        if ((globalThis as unknown as GlobalWithRari)["~rari"].hydrateClientComponents) {
          const hydrateFn = (globalThis as unknown as GlobalWithRari)["~rari"]
      ...
      function rscToHtml(element: any): string {
        ...
        if (Array.isArray(element)) {
          if (element.length >= 4 && element[0] === "$") {
            ...
            const sanitizedTag =
              typeof tag === "string" && RSC_ALLOWED_TAGS.has(tag.toLowerCase())
                ? tag.toLowerCase()
                : "div";
            ...
            if (props) {
              ...
              if (innerHTML === null && props.children) children = rscToHtml(props.children);
            }
  ...
  if (
    ...
    windowWithRari["~rari"].streaming!.bufferedEvents!.length > 0
  ) {
  ...
  const componentName =
    exportName === "default"
      ? componentFunction.name ||
        id.split("/").pop()?.replace(EXTENSION_REGEX, "") ||
        "DefaultComponent"
      : exportName;
...
let createFromReadableStream: typeof rariCreateFromReadableStream | null =
  rariCreateFromReadableStream;
let rscClientLoadPromise: Promise<any> | null = null;

async function loadRscClient(): Promise<any> {
  if (rscClientLoadPromise) return rscClientLoadPromise;
  ...
  rscClientLoadPromise = (async () => {
    try {
      ...
      if (typeof createFromReadableStream !== "function") createFromReadableStream = null;
      if (typeof createFromFetch !== "function") createFromFetch = null;
...
class RscClient {
  ...
  async fetchServerComponent(componentId: string, props: any = {}): Promise<any> {
    const hmrCounter =
      (typeof window !== "undefined" &&
        (window as unknown as WindowWithRari)["~rari"]?.hmr?.refreshCounters?.[componentId]) ||
      0;
    const cacheKey = `${componentId}:${JSON.stringify(props)}:hmr:${hmrCounter}`;

    if (this.componentCache.has(cacheKey)) return this.componentCache.get(cacheKey);
  ...
  async fetchServerComponentStream(componentId: string, props: any = {}): Promise<any> {
    ...
    const endpoints = (() => {
      ...
      try {
        ...
        if (isLocalHost) {
          ...
          if (serverUrl) list.push(`${serverUrl}/_rari/stream`);
        }
      } catch {}
    ...
    if (!response) throw lastError || new Error("Failed to reach stream endpoint");
    ...
    if (!stream) throw new Error("No ReadableStream from stream response");
            ...
            if (Array.isArray(child)) {
              processedProps.children = child.map((c, i) => {
                if (isValidElement(c) && c.key == null) {
                  // oxlint-disable-next-line react/no-clone-element, react/no-array-index-key
              ...
              if (mod) {
                ...
                if (clientComponent) {
                  const reactElement = createElement(
                    clientComponent,
                    key ? { ...processedProps, key } : processedProps,
                  );
                  return reactElement;
                }
              }
    ...
    const StreamingWrapper = (): any => {
      ...
      const renderWithBoundaryUpdates = (element: any): any => {
        ...
        if (Array.isArray(element)) return element.map((child) => renderWithBoundaryUpdates(child));
  ...
  parseRscResponse(rscPayload: string): any {
    ...
    if (errors.length > 0)
      throw new Error(
        `RSC Server Error: ${errors.map((e) => e?.error ?? e?.message ?? e).join(", ")}`,
      );
    ...
    for (const key of elementKeys) {
      ...
      if (Array.isArray(element) && element.length >= 2 && element[0] === "$") {
        ...
        const boundaryId = props?.["~boundaryId"];
        const resolvedType =
          typeof type === "string" && type.startsWith("$") ? symbols.get(type) : undefined;
        if ((isSuspenseType(type) || isSuspenseType(resolvedType)) && props && boundaryId) {
          continue;
        }
  ...
  reconstructElementFromRscData(
    elementData: any,
    modules: Map<string, ModuleData>,
    symbols?: Map<string, string>,
  ): any {
    if (elementData === null || elementData === undefined) return null;

    if (
      typeof elementData === "string" ||
      typeof elementData === "number" ||
      typeof elementData === "boolean"
    )
      return elementData;
    ...
    if (Array.isArray(elementData)) {
      if (elementData.length >= 4 && elementData[0] === "$") {
          ...
          if (modules.has(type)) {
            ...
            if (moduleData) {
              ...
              if (clientComponent) {
                ...
                if (process.env.NODE_ENV !== "production") {
                  console.warn(
                    `[rari] Missing client component: ${moduleData.name} (${moduleData.id})`,
                  );
                }
                actualType = PassthroughComponent;
              }
        ...
        const resolvedType =
          typeof type === "string" && type.startsWith("$") && !type.startsWith("$L")
            ? symbols?.get(type)
            : undefined;
        if (isSuspenseType(type) || isSuspenseType(resolvedType)) {
          actualType = Suspense;
        }
        ...
        return elementData.map((item) =>
          this.reconstructElementFromRscData(item, modules, symbols),
        );
      }
    }

    if (typeof elementData === "object") return null;

    return elementData;
  }

  processPropsRecursively(
    props: any,
    modules: Map<string, ModuleData>,
    symbols?: Map<string, string>,
  ): any {
      ...
      if (key === "children") {
        if (Array.isArray(value)) {
          if (value.length >= 2 && value[0] === "$") {
            ...
            const processedChildren = value
              .map((child) => this.reconstructElementFromRscData(child, modules, symbols))
              .filter((child) => child !== null && child !== undefined);

            if (processedChildren.length === 0) processed[key] = null;
            else if (processedChildren.length === 1) processed[key] = processedChildren[0];
            else processed[key] = processedChildren;
          }
        } else {
...
function RscErrorComponent({ error, details }: { error: string; details?: any }): any {
  if (process.env.NODE_ENV !== "production") {
    return createElement(
      ...
      details && createElement("pre", null, JSON.stringify(details, null, 2)),
    );
  }
...
function ServerComponentWrapper({
  ...
  if (!serverComponentCache.has(cacheKey)) {
    const promise = rscClient.fetchServerComponent(componentId, props).catch((err) => {
  ...
  if (data?.["~isRscResponse"]) {
    ...
    return createElement(
      ...
      createElement(RscRootReader, { rootPromise }),
    );
  }
...
> {
  ...
  componentDidCatch(error: Error, errorInfo: any) {
    console.error(
      `[rari] ServerComponentErrorBoundary: Error in component ${this.props.componentId}:`,
      error,
      errorInfo,
    );
  }
...
function createServerComponentWrapper(componentName: string): (props: any) => any {
  ...
  if (typeof window !== "undefined") {
    const windowWithRari = window as unknown as WindowWithRari;
    if (!windowWithRari["~rari"].hmr) windowWithRari["~rari"].hmr = { refreshCounters: {} };
      ...
      const handleRscInvalidate = (event: Event) => {
        ...
        if (detail && detail.filePath && isServerComponent(detail.filePath)) {
          ...
          if (typeof window !== "undefined") {
            ...
            windowWithRari["~rari"].hmr!.refreshCounters[componentName] =
              (windowWithRari["~rari"].hmr!.refreshCounters[componentName] || 0) + 1;
...
function isServerComponent(filePath: string): boolean {
  ...
  try {
    if (
      ...
      (globalThis as unknown as GlobalWithRari)["~rari"].serverComponents
    )
...
if (import.meta.hot) {
  ...
  registerHandler("rari:register-server-component", (data) => {
    if (data?.filePath) {
      if (typeof globalThis !== "undefined") {
        (globalThis as unknown as GlobalWithRari)["~rari"].serverComponents =
          (globalThis as unknown as GlobalWithRari)["~rari"].serverComponents || new Set();
  ...
  registerHandler("rari:app-router-updated", async (data) => {
    try {
      ...
      if (!data.routePath && (!data.affectedRoutes || data.affectedRoutes.length === 0)) return;
  ...
  async function handleAppRouterUpdate(data: any): Promise<void> {
    ...
    if (metadataChanged && metadata) updateDocumentMetadata(metadata);
    ...
    if (manifestUpdated) await reloadAppRouterManifest();
  ...
  function clearCacheForRoutes(routes: string[]): void {
    ...
    for (const key of serverCacheKeysToDelete) serverComponentCache.delete(key);
  }
    ...
    if (componentId || filePath) {
      try {
        ...
        if (!invalidateResponse.ok) {
          console.error(
            "[rari] HMR: Server cache invalidation failed:",
            invalidateResponse.status,
            responseBody,
          );
        }
      } catch (error) {
  ...
  async function reloadAppRouterManifest(): Promise<void> {
    if (
      typeof window !== "undefined" &&
      (window as unknown as WindowWithRari)["~rari"]?.routeInfoCache
    ) {
      const windowWithRari = window as unknown as WindowWithRari;
...
class HMRErrorOverlay {
  ...
  show(error: any): void {
    ...
    if (this.overlay) this.updateOverlay(error);
    else this.createOverlay(error);
  }
  ...
  updateOverlay(error: any): void {
    ...
    container.style.cssText =
      "position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0, 0, 0, 0.85); z-index: 999999; display: flex; align-items: center; justify-content: center; padding: 2rem; backdrop-filter: blur(4px);";

    const content = document.createElement("div");
    content.style.cssText =
      "background: #1e1e1e; color: #e0e0e0; border-radius: 0.5rem; padding: 2rem; max-width: 50rem; width: 100%; max-height: 90vh; overflow-y: auto; box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.5), 0 10px 10px -5px rgba(0, 0, 0, 0.4); border: 1px solid #ef4444;";
    ...
    closeButton.style.cssText =
      "background: transparent; border: none; color: #9ca3af; cursor: pointer; padding: 0.5rem; border-radius: 0.25rem; transition: all 0.2s; font-size: 1.5rem; line-height: 1; width: 2rem; height: 2rem; display: flex; align-items: center; justify-content: center;";
    ...
    if (error.filePath) {
      ...
      fileInfo.style.cssText =
        "margin-bottom: 1rem; padding: 0.75rem; background: rgba(0, 0, 0, 0.2); border-radius: 0.375rem; font-family: monospace; font-size: 0.875rem;";
    ...
    errorTitle.style.cssText =
      "margin: 0 0 0.75rem 0; font-size: 1rem; font-weight: 600; color: #fca5a5;";
    errorTitle.textContent = "Error Message:";

    const errorPre = document.createElement("pre");
    errorPre.style.cssText =
      "margin: 0; padding: 1rem; background: rgba(239, 68, 68, 0.1); border-left: 4px solid #ef4444; border-radius: 0.375rem; overflow-x: auto; font-family: monospace; font-size: 0.875rem; line-height: 1.5; white-space: pre-wrap; word-break: break-word; color: #fca5a5;";
    ...
    if (error.stack) {
      ...
      stackPre.style.cssText =
        "margin: 0; padding: 0.75rem; background: rgba(0, 0, 0, 0.2); border-radius: 0.375rem; overflow-x: auto; font-size: 0.875rem; line-height: 1.5; white-space: pre-wrap; word-break: break-word;";
    ...
    footer.style.cssText =
      "margin-top: 1.5rem; padding-top: 1.5rem; border-top: 1px solid #374151; display: flex; gap: 0.75rem; align-items: center;";

    const reloadButton = document.createElement("button");
    reloadButton.style.cssText =
      "padding: 0.625rem 1.25rem; background: #ef4444; color: white; border: none; border-radius: 0.375rem; cursor: pointer; font-weight: 600; font-size: 0.875rem; transition: all 0.2s;";
    ...
    dismissButton.style.cssText =
      "padding: 0.625rem 1.25rem; background: #374151; color: #e0e0e0; border: none; border-radius: 0.375rem; cursor: pointer; font-weight: 600; font-size: 0.875rem; transition: all 0.2s;";
    ...
    timestamp.textContent = !Number.isNaN(timestampDate.valueOf())
...
function getErrorOverlay(): HMRErrorOverlay {
  if (!hmrErrorOverlay) hmrErrorOverlay = new HMRErrorOverlay();
...
if (import.meta.hot) {
  ...
  import.meta.hot.on("rari:hmr-error", (data) => {
    ...
    if (filePath) console.error("[rari] HMR: File:", filePath);

    if (data.stack) console.error("[rari] HMR: Stack:", data.stack);
    ...
    if (errorCount && maxErrors) {
      if (errorCount >= maxErrors)
        console.error(
          `[rari] HMR: Maximum error count (${maxErrors}) reached. Consider restarting the dev server if issues persist.`,
        );
      else if (errorCount >= maxErrors - 2)
        console.warn(
          `[rari] HMR: Error count: ${errorCount}/${maxErrors}. Approaching maximum error threshold.`,
        );
    }
  });
...
export { createServerComponentWrapper, rscClient, RscErrorComponent };
