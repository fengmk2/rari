...
export function getComponentFromInfo(componentInfo: LazyComponentInfo, exportName?: string): any {
  if (componentInfo.component == null) return null;

  const module = componentInfo.component;

  if (!exportName || exportName === "default") return module.default ?? module;

  if (module[exportName] !== undefined) return module[exportName];

  if (module.default?.[exportName] !== undefined) return module.default[exportName];
...
function tryLoadComponent(componentInfo: LazyComponentInfo): void {
  if (componentInfo.loader && !componentInfo.loading && !componentInfo.loadPromise) {
    triggerComponentLoad(componentInfo).catch(() => {
...
function resolveByPath(
  ...
  if (!componentId || !clientComponents[componentId]) return null;
...
function resolveExact(id: string, clientComponents: Record<string, ComponentInfo>): any {
  ...
  const exportName = hashIndex === -1 ? undefined : id.slice(hashIndex + 1);
  const component =
    getComponentFromInfo(componentInfo, exportName) ??
...
export function resolveClientComponent(id: string, globalAccessor: GlobalAccessor): any {
  ...
  const exactResult =
    resolveExact(normalizedId, clientComponents) ||
    (normalizedId !== id ? resolveExact(id, clientComponents) : null);
  if (exactResult !== null) return exactResult;

  const directResult =
    resolveById(normalizedId, clientComponents) ||
    (normalizedId !== id ? resolveById(id, clientComponents) : null);
  if (directResult !== null) return directResult;

  const pathResult =
    resolveByPathWithExport(normalizedId, clientComponents, clientComponentPaths) ||
    (normalizedId !== id
      ? resolveByPathWithExport(id, clientComponents, clientComponentPaths)
      : null);
  if (pathResult !== null) return pathResult;

  return (
    resolveByName(normalizedId, clientComponents, clientComponentNames) ||
...
export async function getClientComponentAsync(id: string): Promise<any> {
  ...
  if (componentInfo) return await ensureComponentLoaded(componentInfo, exportName);
