...
function isValidFlightPayload(content: string): boolean {
  ...
  if (firstCharCode === TAG_TEXT) return true;

  if (firstChar === "[") return true;

  if (firstChar === "{" || content.startsWith("E{")) return true;

  if (firstChar === '"') return true;

  if (PRIMITIVE_JSON_REGEX.test(content)) return true;
...
function isFallbackFlightPayload(content: string): boolean {
  ...
  if (content.includes('"fallback"') || content.includes("$Lfallback")) return true;

  if (content.startsWith("E{")) return true;
...
interface LazyContentProps {
  ...
  suspendingPromisesRef: React.RefObject<
    Map<string, { promise: Promise<never>; timestamp: number }>
  >;
...
function LazyContent({
  ...
  rscToReact,
}: LazyContentProps): React.ReactNode {
...
export function AppRouterProvider({
  ...
  useEffect(() => {
    if (rscPayload?.element != null) {
      const isThenable =
        rscPayload.element &&
        typeof rscPayload.element === "object" &&
      ...
      if (isThenable) {
        ...
        if (status === "rejected") {
          ...
          if (reason) console.error("[rari] AppRouter: Flight payload rejected:", reason);
        }
      }
    }
  }, [rscPayload]);
  ...
  const restoreFormState = () => {
    ...
    forms.forEach((form, index) => {
      ...
      savedData.forEach((value, key) => {
        ...
        if (input) {
          if (input.type === "checkbox" || input.type === "radio") input.checked = value === "on";
  ...
  const isStaleContent = (wireFormat: string): boolean => {
    if (!lastSuccessfulPayloadRef.current) return false;

    if (wireFormat === lastSuccessfulPayloadRef.current) return true;
  ...
  function rscToReact(
    ...
    if (
      ...
      rsc.startsWith("$") &&
      rsc.length > 1 &&
      HEX_REGEX.test(rsc.substring(1))
    ) {
    ...
    if (typeof rsc === "string" || typeof rsc === "number" || typeof rsc === "boolean") return rsc;
      ...
      if (rsc.length >= 4 && rsc[0] === "$") {
        ...
        if (typeof type === "string" && type.startsWith("$") && symbols) {
          ...
          if (NUMERIC_REGEX.test(symbolId)) {
            ...
            if (symbolName) {
              if (isSuspenseType(symbolName)) resolvedType = "Suspense";
              else console.warn("[rari] AppRouter: Unknown symbol:", symbolName);
            }
          }
        }
        ...
        if (
          ...
          return React.createElement(
            React.Suspense,
            serverKey ? { ...processedProps, key: serverKey } : processedProps,
          );
        }
        ...
        if (
          ...
          MODULE_REF_REGEX.test(resolvedType)
        ) {
          const moduleInfo = modules.get(resolvedType);

          if (!moduleInfo) {
            console.warn(
              "[rari] AppRouter: Module not found for reference:",
              resolvedType,
              "Available modules:",
          ...
          const effectiveKey =
            serverKey || `fallback-${resolvedType}-${fallbackKeyCounterRef.current++}`;
          ...
          const childProps = {
            ...
            children:
              props.children === undefined
                ? undefined
                : rscToReact(props.children, modules, layoutPath, symbols, rows),
          };
        ...
        if (
          !resolvedType ||
          (typeof resolvedType !== "string" && typeof resolvedType !== "function")
        )
          return null;

        if (typeof resolvedType === "string" && resolvedType.startsWith("$")) return null;
      ...
      return (rsc as RSCArray)
        .map((child, index) => {
          ...
          if (typeof element === "object" && React.isValidElement(element) && !element.key) {
            const childHash =
              typeof child === "object" && child !== null ? hashRscData(child) : "primitive";
            const childType =
              Array.isArray(child) && child[0] === "$" && child[1] ? String(child[1]) : "unknown";
  ...
  const suspendingPromisesRef = useRef<Map<string, { promise: Promise<never>; timestamp: number }>>(
    new Map(),
  );
  ...
  function cleanupStaleSuspendingPromises() {
    ...
    suspendingPromisesRef.current.forEach((entry, key) => {
      if (now - entry.timestamp > SUSPENSE_TTL_MS) staleEntries.push(key);
    });
  ...
  function processProps(
    ...
    if (rows) rowsDataRef.current = rows;
    if (modules) modulesDataRef.current = modules;
    if (symbols) symbolsDataRef.current = symbols;
      ...
      if (Object.hasOwn(props, propKey)) {
        if (propKey === "children") {
          ...
          if (
            ...
            children.startsWith("$") &&
            children.length > 1 &&
            HEX_REGEX.test(children.substring(1))
          ) {
            ...
            processed[propKey] =
              children === undefined
                ? undefined
                : rscToReact(children, modules, layoutPath, symbols, rows);
          }
  ...
  const refetchRscPayload = async (targetPath?: string, abortSignal?: AbortSignal) => {
    ...
    const fetchPromise = (async () => {
      try {
        const rariServerUrl = (import.meta.env.RARI_SERVER_URL || window.location.origin).replace(
  ...
  const refetchRscPayloadRef =
    useRef<(targetPath?: string, abortSignal?: AbortSignal) => Promise<any>>(refetchRscPayload);
  ...
  useEffect(() => {
    ...
    const handleNavigate = async (event: Event) => {
      ...
      if (!detail.isStreaming) streamingRowsRef.current = null;
      ...
      if (parsedPayload && currentNavigationIdRef.current === detail.navigationId) {
        ...
        if (detail.rscWireFormat) lastSuccessfulPayloadRef.current = detail.rscWireFormat;
    ...
    const handleNavigationStart = (event: Event) => {
      ...
      if (typeof window !== "undefined") {
        ...
        if (!globalWindow["~rari"]) globalWindow["~rari"] = {} as GlobalWithRari["~rari"];
    ...
    const processRows = async () => {
      if (!streamingRowsRef.current || streamingRowsRef.current.length === 0) return;

      if (isInitialPageLoadRef.current) return;
      ...
      const hasPageContent = rows.some((r) => {
        ...
        const id = ci > 0 ? r.substring(0, ci).trim() : "";
        const content = ci > 0 ? r.substring(ci + 1) : "";
        if (!HEX_REGEX.test(id) || !isValidFlightPayload(content)) return false;
        ...
        const isReferencedByShell = rows.some((shellRow) => {
          ...
          if (
            !HEX_REGEX.test(shellId) ||
            !isValidFlightPayload(shellContent) ||
            isFallbackFlightPayload(shellContent)
          )
            return false;
      ...
      if (!hasShellContent && !hasPageContent) return;

      if (!hasRenderedInitialShellRef.current && hasShellContent && !hasPageContent) {
        hasRenderedInitialShellRef.current = true;

        const hasSuspenseBoundary = rows.some(
          (r) => r.includes('"$Sreact.suspense"') || r.includes("react.suspense"),
        );
      ...
      if (!hasPageContent && hasSuspenseBoundary && !streamCompleteRef.current) return;

      if (hasRenderedFinalRef.current) return;
      ...
      try {
        ...
        if (hasSuspenseBoundary || hasPageContent) {
          ...
          if (currentNavigationIdRef.current === navId && !hasRenderedFinalRef.current) {
            ...
            if (currentNavigationIdRef.current === navId) {
              ...
              if (streamCompleteRef.current) streamingRowsRef.current = null;
            }
          }
        }

        if (currentNavigationIdRef.current !== navId) return;
        ...
        if (currentNavigationIdRef.current !== navId || hasRenderedFinalRef.current) return;
    ...
    const handleRscRow = (event: Event) => {
      ...
      if (!streamingRowsRef.current) return;

      const activeNavId = currentNavigationIdRef.current;

      if (eventNavigationId == null || eventNavigationId !== activeNavId) return;

      if (row.trim() === "STREAM_COMPLETE") {
        streamCompleteRef.current = true;
        rowProcessingRef.current = rowProcessingRef.current.then(() => {
          if (currentNavigationIdRef.current === activeNavId) return processRows();
        });
        return;
      }
  ...
  if (
    ...
    React.isValidElement(contentToRender[0])
  )
    contentToRender = contentToRender[0];
  else if (
    Array.isArray(contentToRender) &&
    contentToRender.length > 0 &&
    contentToRender.every(
      (item) =>
  ...
  return (
    <>
      ...
      <Suspense fallback={null}>{contentToRender}</Suspense>
    </>
  );
}
