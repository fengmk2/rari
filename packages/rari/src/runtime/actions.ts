    ...
    try {
      ...
      if (result.redirect) {
        ...
        if (typeof window !== "undefined") {
          ...
          if (absoluteRedirect !== window.location.href) window.location.href = absoluteRedirect;
        }
        /* v8 ignore stop */
  ...
  const handleSubmit = async (event: Event) => {
    ...
    try {
      ...
      if (result && result.redirect) {
        if (options.onRedirect) options.onRedirect(result.redirect);

        if (typeof window !== "undefined") window.location.href = result.redirect;
      ...
      else console.error("Server action error:", errorMessage);
    }
  };
