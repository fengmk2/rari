...
function wakeChunk<T>(listeners: Array<(value: T) => void>, value: T): void {
...
function triggerErrorOnChunk<T>(chunk: SomeChunk<T>, error: any): void {
...
function createResolvedModelChunk<T>(response: Response, value: string): ResolvedModelChunk<T> {
  return new (ReactPromise as any)(RESOLVED_MODEL, value, response);
}

function createResolvedModuleChunk<T>(value: ClientReference<T>): ResolvedModuleChunk<T> {
  return new (ReactPromise as any)(RESOLVED_MODULE, value, null);
}

function createInitializedTextChunk(value: string): InitializedChunk<string> {
  return new (ReactPromise as any)(INITIALIZED, value, null);
}

function resolveModelChunk<T>(response: Response, chunk: SomeChunk<T>, value: string): void {
  if (chunk.status !== PENDING && chunk.status !== BLOCKED) {
    return;
  }
...
function createElement(response: Response, type: any, key: any, props: any): React.ReactElement {
  props ??= {};
...
function createLazyChunkWrapper<T>(chunk: SomeChunk<T>): React.LazyExoticComponent<any> {
  const lazyType: any = {
...
function parseModelString(response: Response, parentObject: any, key: string, value: string): any {
  if (value[0] === "$") {
    if (value === "$") {
    ...
    switch (value[1]) {
      ...
      case "F": {
        ...
        return getOutlinedModel(response, ref, parentObject, key, (model) =>
          loadServerReference(response, model),
        );
      }
      case "Q": {
        const ref = value.slice(2);
...
function parseModelTuple(response: Response, value: Array<any>): any {
  const tuple = value;
...
function reviveModel(response: Response, value: any, parentObject: any, key: string): any {
...
export function createResponse(
  ...
  return new (ResponseInstance as any)(bundlerConfig, callServer);
}
...
function resolveModel(response: Response, id: number, model: string): void {
  const chunks = response._chunks;
...
function resolveModule(response: Response, id: number, model: string): void {
  const chunks = response._chunks;
  const chunk = chunks.get(id);
  const clientReferenceMetadata: ClientReferenceMetadata = parseModel(response, model);
  const clientReference = resolveClientReference<any>(clientReferenceMetadata);
...
function resolveErrorModel(response: Response, id: number, digest: string): void {
...
function processFullStringRow(response: Response, id: number, tag: number, row: string): void {
  switch (tag) {
    case 73: {
      /* "I" */ resolveModule(response, id, row);
      return;
    }
    case 69: {
      /* "E" */ const errorInfo = JSON.parse(row);
      resolveErrorModel(response, id, errorInfo.digest);
      return;
    }
    case 84: {
      /* "T" */ resolveText(response, id, row);
      return;
    }
    default: {
