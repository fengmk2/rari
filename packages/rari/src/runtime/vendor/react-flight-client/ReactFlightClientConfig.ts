...
export function preloadModule<T>(moduleReference: ClientReference<T>): null | Promise<any> {
  ...
  const componentInfo =
    clientComponents[normalizedKey] ||
  ...
  if (componentInfo.loadPromise) return componentInfo.loadPromise;
...
export function requireModule<T>(moduleReference: ClientReference<T>): T {
  ...
  if (componentInfo.component) {
    ...
    if (module[moduleReference.name] !== undefined) return module[moduleReference.name];
  ...
  if (componentInfo.loadPromise) throw componentInfo.loadPromise;
