...
export function Image({
  ...
  useEffect(() => {
    if (shouldPreload) {
      ...
      link.rel = "preload";
      link.as = "image";
      if (loader) link.href = loader({ src: finalSrc, width: imgWidth || 1920, quality });
      else if (unoptimized) link.href = finalSrc;
      else link.href = buildImageUrl(finalSrc, imgWidth || 1920, quality);
  ...
  const buildSrcSet = (format?: ImageFormat) => {
    if (loader)
      return sizesArray
        .map((w) => `${loader({ src: finalSrc, width: w, quality })} ${w}w`)
        .join(", ");
