...
class ResponseCookiesImpl implements ResponseCookies {
  ...
  toSetCookieHeaders(): string[] {
    return Array.from(this.cookies.values(), (cookie) => {
      ...
      if (cookie.options) {
        if (cookie.options.path) header += `; Path=${cookie.options.path}`;
        if (cookie.options.domain) header += `; Domain=${cookie.options.domain}`;
        if (cookie.options.maxAge) header += `; Max-Age=${cookie.options.maxAge}`;
