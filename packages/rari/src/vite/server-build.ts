...
const CODE_IMPORT_REGEX =
  /from\s+['"]([^'"]+)['"]|import\s*\(\s*['"]([^'"]+)['"]\s*\)|import\s+['"]([^'"]+)['"]/g;
const EXTRACT_DEPENDENCIES_REGEX =
...
type ResolvedServerBuildOptions = Required<
  Omit<
    ServerBuildOptions,
    "csp" | "cacheControl" | "cache" | "define" | "serverConfigPath" | "experimental"
  >
> & {
  serverConfigPath: string;
...
export class ServerComponentBuilder {
  ...
  private directiveResultCache = new Map<
    string,
    { hasUseClient: boolean; hasUseServer: boolean; error: boolean }
  >();
  ...
  isServerComponent(filePath: string, source?: string): boolean {
    ...
    if (this.isHtmlOnlyImport(filePath)) return false;

    const directives = this.getDirectivesCached(filePath, source);
    if (directives.error) return false;
  ...
  addServerComponent(filePath: string, source?: string) {
    ...
    if (!this.isServerComponent(filePath, code)) return;
...
`,
          ...
          if (id.startsWith("\0server-action:")) {
            ...
            const actionId =
              serverActionRefs.get(filePath)?.actionId ?? self.getComponentId(filePath);
      ...
      },
      {
        ...
        resolveId: (source: string, importer: string | undefined) => {
          ...
          for (const ext of extensions) {
            const pathWithExt = resolvedPath + ext;
            if (fs.existsSync(pathWithExt) && fs.statSync(pathWithExt).isFile()) {
              if (self.isClientComponent(pathWithExt)) return null;
      ...
      },
      {
        ...
        resolveId: (source: string, importer: string | undefined) => {
          if (source.endsWith(".module.css")) {
            const importerDir =
              !importer?.startsWith("\0") && importer ? path.dirname(importer) : resolveDir;
        ...
        resolveId: (source: string) => {
          ...
          const externalPackageMappings: Record<string, string | null> = {
            ...
            "react-server-dom-rari/server":
              "node_modules/rari/dist/runtime/react-server-dom-shim.mjs",
          };
          ...
          if (source === "rari" || source === "rari/client") return null;
      ...
      },
      {
        ...
        async transform(code: string, id: string) {
          if (!self.options.experimental?.useCache) return null;
  ...
  async buildServerComponents(): Promise<ServerComponentManifest> {
    ...
    if (Object.keys(serverConfig).length === 0) {
      try {
        ...
        if (e.code !== "ENOENT") console.warn(`Failed to remove server config file:`, error);
      }
    } else {
      await fs.promises.writeFile(serverConfigPath, JSON.stringify(serverConfig, null, 2), "utf-8");
    }
    ...
    const scanForClientComponents = (dir: string) => {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          try {
            const code = fs.readFileSync(fullPath, "utf-8");
            if (hasTopLevelUseClientDirective(code)) clientFiles.push({ filePath: fullPath, code });
          } catch {}
        }
      }
    };
  ...
  private async buildSSRSingleClient(
    ...
    const result = await build({
      ...
      platform: "node",
      write: false,
      external: [NODE_PROTOCOL_REGEX, "react", "react-dom", /^react\//, /^rari/],
      output: {
        format: "esm",
        minify: false,
      },
        ...
        },
        {
          ...
          resolveId(id, importer) {
            if (id.startsWith(".") || id.startsWith("/") || id.startsWith("@/")) {
              let resolved = id;
              if (id.startsWith("@/")) resolved = path.join(projectRoot, "src", id.slice(2));
              ...
              const found =
                resolveWithExtensions(resolved, [".ts", ".tsx", ".js", ".jsx"]) ||
...
export function isEligibleServerComponent(
  ...
  if (hasTopLevelUseClientDirective(code)) return false;

  if (hasTopLevelUseServerDirective(code)) return true;
...
export function scanDirectory(dir: string, builder: ServerComponentBuilder, isTopLevel = true) {
  if (isTopLevel) builder.buildImportGraph(dir);
  ...
  return {
    ...
    async handleHotUpdate({ file }) {
      ...
      try {
        ...
        if (!isTracked) builder.addServerComponent(file, content);
