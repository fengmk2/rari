...
const NAMED_IMPORT_REGEX =
  /import\s+\{[^}]*\b(?:Image\s+as\s+(\w+)|Image)\b[^}]*\}\s+from\s+['"]rari\/image['"]/g;
...
export async function scanForImageUsage(
  srcDir: string,
...
async function transformCode(
  ...
  if (!result.output || result.output.length === 0) throw new Error("Transform produced no output");
...
function parseImageImports(code: string): Set<string> {
  ...
  for (const match of code.matchAll(DEFAULT_IMPORT_REGEX)) identifiers.add(match[1]);

  for (const match of code.matchAll(NAMED_IMPORT_REGEX)) {
    if (match[1]) identifiers.add(match[1]);
    else identifiers.add("Image");
  }
...
function addImageToMap(imageUsage: ImageUsage, images: Map<string, ImageUsage>): void {
  ...
  if (!images.has(key) || imageUsage.preload) images.set(key, imageUsage);
}
  ...
  for (let i = startIndex; i < code.length; i++) {
    ...
    if (!inString) {
      if (char === "{") {
        ...
        braceCount--;
        if (braceCount === 0) return code.substring(startIndex + 1, i);
      }
    }
  }
...
function processImageIdentifiers(
  ...
  for (const identifier of imageIdentifiers) {
    ...
    const createElementPattern = new RegExp(
      `React\\.createElement\\(\\s*${escapedIdentifier}\\s*,\\s*\\{`,
      "g",
    );
    ...
    for (const match of transformedCode.matchAll(createElementPattern)) {
      ...
      if (propsString) {
        ...
        if (imageUsage) addImageToMap(imageUsage, images);
      }
    }
  }
}
...
async function extractImageUsages(
  content: string,
  filePath: string,
  images: Map<string, ImageUsage>,
) {
  ...
  try {
    ...
    if (imageIdentifiers.size === 0) return;
...
function extractImageUsagesWithRegex(
  content: string,
  aliases: Set<string>,
  images: Map<string, ImageUsage>,
) {
  for (const alias of aliases) {
    ...
    for (const match of content.matchAll(selfClosingRegex)) processImageProps(match[1], images);

    for (const match of content.matchAll(openingRegex)) processImageProps(match[1], images);
  }
}
...
function processImageProps(propsString: string, images: Map<string, ImageUsage>) {
  ...
  if (!src || src.includes("{") || (!src.startsWith("/") && !src.startsWith("http"))) return;
  ...
  const preload =
    PRELOAD_PROP_REGEX.test(propsString) && !PRELOAD_FALSE_PROP_REGEX.test(propsString);
