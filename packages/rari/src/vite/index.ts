...
export function rari(options: RariOptions = {}): Plugin[] {
  ...
  function getModuleDirectives(id: string): { hasUseServer: boolean; hasUseClient: boolean } {
    if (directiveCache.has(id)) return directiveCache.get(id)!;
  ...
  function getHtmlEntryImports(): Set<string> {
    ...
    try {
      ...
      if (htmlEntryImports === null) htmlEntryImports = new Set();
  ...
  function isServerComponent(filePath: string): boolean {
    ...
    if (getHtmlEntryImports().has(filePath)) return false;
    ...
    try {
      ...
      if (directives.hasUseServer) return false;
  ...
  function parseExportedNames(code: string): string[] {
    try {
      ...
      else if (hasDefaultExport(code)) exportedNames.push("default");
...
if (import.meta.hot) {
  ...
  function transformClientModule(code: string, id: string): string {
    ...
    if (isServerComp) {
      ...
      let newCode = 'import { createServerComponentWrapper } from "virtual:rsc-integration.ts";\n';
    ...
    if (!hasTopLevelUseClientDirective(code)) return code;
  ...
  function resolveImportToFilePath(importPath: string, importerPath: string): string {
  ...
  const mainPlugin: Plugin = {
    ...
    config(config: UserConfig, { command }) {
      ...
      if (Array.isArray((config.resolve as any).alias)) {
        ...
        typeof (config.resolve as any).alias === "object"
      ) {
    ...
    async transform(code, id) {
      ...
      if (isServerComponent(id)) {
        ...
        if (environment && (environment.name === "rsc" || environment.name === "ssr")) {
...
${clientTransformedCode}`;
      ...
      if (cachedType === "client") return transformClientModuleForClient(code, id);
      ...
      const importingFileIsClient =
        hasTopLevelUseClientDirective(code) ||
...
const ${componentName} = registerClientReference(
  ...
  ${JSON.stringify(importedDefault || "default")}
      ...
      if (hasServerImports) {
        ...
        const hasWrapperImport = modifiedCode.includes("createServerComponentWrapper");
        ...
        if (!hasTopLevelUseClientDirective(modifiedCode) && hasJsx) {
          if (isDevMode) {
    ...
    async configureServer(server) {
        ...
        try {
          ...
          for (const component of components) {
            try {
              ...
              if (hasTopLevelUseServerDirective(component.code)) continue;
      ...
      const ensureClientComponentsRegistered = async () => {
        try {
          ...
          const clientComponentFiles = scanForClientComponents(
            srcDir,
            Object.values(resolvedAlias),
          );
          ...
          if (!rustServerReady) {
            ...
            if (!isHealthy) {
              ...
              while (Date.now() - startWait < maxWait) {
                if (await checkRustServerHealth()) break;
      ...
      server.watcher.on("change", async (filePath) => {
        if (TSX_EXT_REGEX.test(filePath)) componentTypeCache.delete(filePath);
    ...
    async load(id) {
      ...
      if (id === "virtual:rari-entry-client.ts") {
        ...
        const scannedClientComponents = scanForClientComponents(
          srcDir,
          Object.values(resolvedAlias),
        );

        const allClientComponents = new Set([...clientComponents, ...scannedClientComponents]);
...
`;
      ...
      if (id === "virtual:app-router-provider.tsx") {
        ...
        if (runtimeFile) return fs.readFileSync(runtimeFile, "utf-8");
...
import * as React from 'react';\n${content}`;
    ...
    async handleHotUpdate({ file, server }) {
      ...
      if (componentType === "client") return;
...
export function defineRariConfig(config: UserConfig & { plugins?: Plugin[] }): UserConfig {
  return {
