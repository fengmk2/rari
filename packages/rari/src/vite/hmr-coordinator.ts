...
export class HMRCoordinator {
  ...
  constructor(builder: ServerComponentBuilder, serverPort: number = 3000) {
  ...
  async handleClientComponentUpdate(filePath: string, server: ViteDevServer): Promise<void> {
    ...
    try {
      ...
      this.queueLog(
        "error",
        `Failed to update client component: ${relativePath} - ${errorMessage}`,
      );
  ...
  private flushLogs(): void {
    ...
    const grouped = this.logBatch.reduce(
      (acc, log) => {
