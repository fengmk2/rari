...
`;
      ...
      const resultB = useCacheAddon.transformUseCache(srcB, {
        ...defaultOpts,
        filename: "test.js",
      });

      const idA = resultA.code.match(
        /registerServerReference\(\$\$RSC_SERVER_CACHE_0_getData, "([^"]+)"/,
      )?.[1];
      const idB = resultB.code.match(
        /registerServerReference\(\$\$RSC_SERVER_CACHE_0_fetchData, "([^"]+)"/,
      )?.[1];
...
`;
    ...
    const result = await p.call({ environment: { name: "rsc" } }, source, filePath);
...
`;
    ...
    let result;
    try {
      result = await p.call({ environment: { name: "rsc" } }, source, filePath);
      expect(result).not.toBeNull();
      expect(result).not.toContain("$$reactCache__");
    } finally {
...
`;
    ...
    const p = mainPlugin.transform as any;
    const result = await p.call({ environment: { name: "client" } }, source, filePath);
...
`;
    const p = mainPlugin.transform as any;
    const result = await p.call({ environment: { name: "rsc" } }, source, "/test/plain.css");
