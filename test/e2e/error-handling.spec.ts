...
test.describe("Error Handling", () => {
  test.describe("Error Boundaries", () => {
    ...
    test("should allow error recovery with reset button", async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="error-boundary"]')).toBeHidden();
    });
  ...
  test.describe("Layout Error Boundaries", () => {
    test("should catch layout errors", async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="error-layout-page"]')).toBeVisible();
    ...
    test("should allow layout error recovery", async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="layout-error-boundary"]')).toBeHidden();
    });
  });
  ...
  test.describe("Nested Error Boundaries", () => {
    test("should catch errors in nested routes", async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="nested-error-child-page"]')).toBeVisible();

      await page.click('[data-testid="trigger-nested-error-button"]');

      await expect(page.locator('[data-testid="nested-error-boundary"]')).toBeVisible();
      await expect(page.locator('[data-testid="nested-error-message"]')).toContainText(
        "Error from nested child page",
      );
    });

    test("should isolate errors to nearest error boundary", async ({ page }) => {
      await page.goto(`/nested-error/child`);
    ...
    test("should allow nested error recovery", async ({ page }) => {
      ...
      await expect(page.locator('[data-testid="nested-error-boundary"]')).toBeHidden();
    });
  });
  ...
  test.describe("404 Not Found", () => {
    ...
    test("should return 404 status for non-existent routes", async ({ request }) => {
  ...
  test.describe("Error Boundary Rendering", () => {
    ...
    test("should render layout error boundary UI correctly", async ({ page }) => {
      await page.goto(`/error-layout`);
      await page.click('[data-testid="trigger-layout-error-button"]');

      const layoutErrorBoundary = page.locator('[data-testid="layout-error-boundary"]');
      await expect(layoutErrorBoundary).toBeVisible();

      await expect(layoutErrorBoundary.locator("h2")).toContainText("Layout Error Caught");
